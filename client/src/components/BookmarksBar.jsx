...
export default function BookmarksBar({ onOpenSettings }) {
  ...
  return (
    <div className="relative w-full bg-[#080b13]/95 border-b border-slate-800/80 backdrop-blur-md px-4 py-1.5 flex items-center justify-between text-xs z-40 transition-all shadow-sm overflow-visible">
      ...
      {/* 1. LEFT: Apps & Other Bookmarks Button */}
      <div className="relative flex items-center flex-shrink-0" ref={appsMenuRef}>
        <button
          onClick={() => {
            setIsAppsOpen(!isAppsOpen);
            setActiveFolderId(null);
          }}
          className={`flex items-center gap-1.5 px-2 py-1 rounded-lg transition-colors cursor-pointer text-xs font-semibold ${
            isAppsOpen 
          ...
          title="Apps & Other Bookmarks"
        >
          <LayoutGrid className="w-3.5 h-3.5" />
          <span className="hidden sm:inline">Apps</span>
        </button>

        {/* APPS & OTHER BOOKMARKS FLOATING PANEL */}
        {isAppsOpen && (
          <div className="absolute top-full left-0 mt-2 w-80 sm:w-96 max-h-[80vh] flex flex-col rounded-2xl bg-[#101726]/98 border border-slate-700/90 shadow-2xl p-3 z-50 animate-slide-up text-left backdrop-blur-2xl ring-1 ring-white/10">
            ...
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="font-bold text-white text-xs">Apps & Other Bookmarks</h4>
            ...
            {/* Quick Chrome Apps Shortcut */}
            <a
              href="chrome://apps"
              onClick={(e) => {
                if (!window.chrome?.bookmarks) e.preventDefault();
              }}
              className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-slate-800/50 hover:bg-slate-800 border border-slate-700/60 text-slate-200 hover:text-white transition-colors text-xs font-semibold mb-2 group cursor-pointer"
            >
              <div className="w-6 h-6 rounded-lg bg-gradient-to-tr from-cyan-500 to-blue-500 flex items-center justify-center text-slate-950 font-bold">
                <LayoutGrid className="w-3.5 h-3.5" />
              </div>
              <div className="flex-1">
                <div className="leading-none">Chrome Apps</div>
                <div className="text-[10px] text-slate-400 font-normal">chrome://apps</div>
              </div>
              ...
              {otherBookmarks.length === 0 ? (
                <div className="p-4 text-center text-xs text-slate-400 bg-slate-800/30 rounded-xl border border-slate-800/80 my-2">
                  ...
                  <p className="text-[11px] text-slate-500 mb-3">
                    Import your full Chrome bookmarks HTML in Settings (⚙️) to populate this menu!
                  </p>
                  <button
                    onClick={() => {
                      setIsAppsOpen(false);
                ...
                filteredOtherBookmarks.map((node) => (
                  <BookmarkTreeItem
                    ...
                    onLinkClick={() => setIsAppsOpen(false)}
                  />
                ))
              )}
            </div>
      ...
      {/* 3. RIGHT SPACER to balance Apps button */}
...
// Recursive Bookmark Item inside Apps Menu (Accordion for Folders)
